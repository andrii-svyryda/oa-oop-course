...
- Зверніть увагу, що поведінкові патерни розподіляють обов’язки між уже створеними об’єктами.
- Спостерігач у самій мові C# — це `event`. Ми детальніше розберемо BCL на лекції 09.
- Стратегію ви вже бачили, коли ми обговорювали SOLID. Пам’ятайте різницю: стан сам змінює свій режим, тоді як стратегію підставляє клієнт.
- Декоратор завжди обгортає виклик, а от ланцюжок обов’язків може взагалі зупинити запит.
- Медіатор чудово зменшує зв’язність, але легко стає «бог-класом» — ви побачите це на лабораторній.

## Спробуйте самі

- **Спостерігач**: реалізуйте товар і підписників із методами `Register` та `Remove`.
- **Команда**: спробуйте інкапсулювати увімкнення світла й додати `Undo`.
- **Стратегія**: напишіть стратегію знижок — це те саме сімейство алгоритмів, що ми робили в лекції про SOLID.
- **Стан**: реалізуйте стан замовлення, де `Draft.Pay` стає `Paid`, а повторний `Pay` уже не діє.
- **Шаблонний метод**: зробіть спільний метод `Run`, який викликає різні реалізації `Read`.
- **Ланцюжок обов’язків**: побудуйте ланцюжок Auth → Role, де без ролі ми далі не йдемо.
- **Посередник**: напишіть спрощений MediatR у лабораторній роботі.

Ітератор і `foreach` ми розглянемо на лекції 09, тому тут вони не виділені в окрему тему.
...
Патерн **Стан** (State) дуже схожий на стратегію: ви маєте різні класи з одним інтерфейсом. Головна різниця полягає в тому, *хто* саме змінює поведінку. Стратегію підставляє клієнт. А от стан *сам* вирішує, який крок буде наступним: наприклад, після виклику `Pay` замовлення вже стає `Paid`, і повторний `Pay` воно просто ігнорує або кидає виняток.
...
**Шаблонний метод** (Template Method) тримає загальний каркас алгоритму в базовому класі. А самі кроки ви робите `abstract` або `virtual` у класах-нащадках. Ви вже бачили таке наслідування на нашій першій лекції. До речі, default interface methods у C# — це слабший варіант того самого підходу.
...
abstract class DataImporter
{
	...
	protected virtual void Parse() { /* спільна логіка */ }
}
```

## Ланцюжок обов’язків

Тут ваш запит іде по руках. Хтось його обробив — або передав далі. Зверніть увагу, що на відміну від декоратора, ланка може *зупинити* ланцюг: якщо немає потрібної ролі, ми до аудиту просто не доходимо.
...
abstract class Handler
{
	...
	public virtual void Handle(string request) => Next?.Handle(request);
}
```

## Коли який поведінковий патерн обрати

- Якщо у вас багато слухачів однієї події — обирайте спостерігач (Observer).
- Коли запит стає окремою річчю (для undo чи черги) — це команда (Command).
- Якщо клієнт обирає алгоритм — це стратегія (Strategy). А коли об’єкт сам змінює свій режим — це стан (State).
- Маєте спільні кроки, але різні деталі — використовуйте шаблонний метод (Template Method).
- Потрібен фільтр чи обробка по черзі з можливістю зупинки — беріть ланцюжок обов’язків (Chain of Responsibility).
- Для зв’язку багато-до-багатьох без зайвої павутини — вам допоможе посередник (Mediator).

Патерни Visitor, Memento та Interpreter ми в лекціях не розглядаємо, але ви можете взяти їх на [доповідь про патерн поза курсом](../labs/general/lab-16-extra-pattern-talk.md).

Код до сьогоднішніх слайдів ви знайдете тут: `code/lecture-08-gof-behavioral`.
