...
Зверніть увагу на різницю між типами значень та типами посилань. Ви зрозумієте, як працює приведення типів, що таке boxing та unboxing, і чому generics є необхідними для ефективного коду. Ми також розглянемо, як створювати узагальнені типи, методи, класи й інтерфейси, та як правильно застосовувати обмеження для узагальнень.

## Спробуйте самі

Порівняйте роботу `List<object>` та `List<int>`, щоб на практиці побачити вплив cast і boxing. Напишіть узагальнений метод для обміну значеннями двох змінних та створіть калькулятор, який підтримує різні числові типи. Спробуйте реалізувати власний шаблонний список, наприклад List або LinkedList. Дослідіть стандартні колекції як приклад використання generics (детальніше про вибір структури даних ви дізнаєтеся на 11-й лекції). Також попрактикуйтеся із шаблонними делегатами Func та Action і застосуйте різні обмеження, такі як notnull, class, default, Enum та new().
...
Приклади коду до цієї лекції ви знайдете в директорії `code/lecture-02-generics`.

---

# Узагальнення в C# 🎯

Узагальнення дають один інтерфейс, клас, метод чи делегат на багато типів. Не треба копіювати код під кожен `int` і `string`. Компілятор сам перевіряє `T`, тож зайвий cast зникає.

Але спочатку розберіть value type, приведення і boxing. Інакше фраза «немає boxing / зайвого cast» нічого не пояснює.

## Типи значень і типи посилань

**Value type** (`int`, `bool`, `double`, `struct`, `enum`) — значення лежить у самій змінній. Присвоєння копіює дані: дві змінні тримають два незалежні `42`.

**Reference type** (`class`, масиви, більшість типів BCL) — змінна тримає посилання на об’єкт на heap. Присвоєння копіює посилання, не об’єкт. `string` — reference type, але незмінний.

Будь-що в C# можна покласти в змінну `object`. Для value type це не безкоштовно — саме тут з’являється boxing.

## Приведення типів (cast)

- **Неявне** — нічого не втрачаємо: `int` → `long`, нащадок → батько (upcast, лекція 01).
- **Явне** — ви берете відповідальність: `(int)x`, батько → нащадок (downcast).
- Не той тип у runtime — `InvalidCastException`.

```csharp
long wide = 42;        // неявно: int → long
int narrow = (int)wide; // явно: можна втратити дані

object box = 10;
int n = (int)box;      // ок, якщо всередині int
string s = (string)box; // виняток у runtime
```

Це звичайне приведення. Власні `implicit` / `explicit operator` між вашими типами будуть у лекції 10 — не плутайте їх з boxing.

## Boxing і unboxing

**Boxing** — value type кладуть в об’єкт на heap. **Unboxing** — дістаєте значення назад; потрібен точний тип. Ціна: виділення пам’яті, перевірка типу, cast.

```csharp
int n = 42;
object box = n;        // boxing
int copy = (int)box;   // unboxing
long fail = (long)box; // InvalidCastException: у коробці int, не long
```

Без generics колекція часто тримає `object`. Тоді кожен `int` бокситься на вході, а на виході потрібен cast — і компілятор не зупинить рядок замість числа:

```csharp
var items = new List<object>(); // раніше так само працював ArrayList
items.Add(10);      // boxing: int → object
items.Add("oops");  // компілятор мовчить
int x = (int)items[0]; // unboxing + cast — ок
int y = (int)items[1]; // бум: рядок не є int
```

`List<int>` зберігає `int` як `int`: без boxing, без cast, з помилкою вже на компіляції, якщо передати `string`.

Тепер видно, навіщо generics:

- безпека типів уже на компіляції
- краща продуктивність (немає boxing і зайвого cast)
- один алгоритм на багато типів

Колекції `List<T>`, `Dictionary<TKey,TValue>`, `Queue<T>`, `Stack<T>` — головний приклад generics, не окрема тема. Яку структуру обрати — лекція 11. Делегати `Func<>` / `Action<>` теж generic; ви вже бачили їх у лекції 01.
